...
def gradient_descent(x_train, y_train, num_iters, learning_rate, theta, corr_class, reg_para):
    X = np.array(x_train)
    y_train=(y_train==corr_class)
    Y = np.array(y_train)
    parameter = theta
    while (num_iters > 0):
        parameter = step_gradient(X, Y, learning_rate, parameter, reg_para)
        num_iters -= 1
    return parameter


def step_gradient(X, Y, learning_rate, parameter, reg_para):
    # print(np.shape(X))
    # print(np.shape(Y))
    ...
    # print(np.shape(temp))
    parameter -= learning_rate * (1 / len(X)) * temp + reg_para*(1/len(X))*parameter
    return parameter
...
for i in range(numclass):
    parameter[i]=gradient_descent(x_train, y_train, num_iters, learning_rate, parameter[i], i, reg_para)
...
# use this for variation with reg_parameter
