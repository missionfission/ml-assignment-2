...
def readfile(file):
    ...
    data1=np.ones([len(data), len(data[0])+4])
    for i in range(len(data1)):
        for j in range(len(data[0])):
            data1[i][j+1]=data[i][j]
            data1[i][j+2]=np.square(data[i][j])
            data1[i][j+3]=np.square(data[i][j])*data[i][j]
            data1[i][j+4]=np.square(np.square(data[i][j]))
    data=data1
    # constant appende
    return data
